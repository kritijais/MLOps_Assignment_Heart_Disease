...
class HeartDiseaseFeatures(BaseModel):
    ...
    restecg: int = Field(..., description="Resting electrocardiographic results (0, 1, or 2).")
    ...
    slope: int = Field(..., description="The slope of the peak exercise ST segment (1, 2, or 3).")
    ca: int = Field(..., description="Number of major vessels (0-3) colored by flourosopy.")
    thal: int = Field(..., description="Thalium stress test result (3 = normal; 6 = fixed defect; 7 = reversable defect).")
