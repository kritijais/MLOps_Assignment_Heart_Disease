...
def generate_and_log_eda(df: pd.DataFrame, artifact_dir: str = "eda_artifacts"):
    ...
    # Categorical columns must be handled, here we use only numerical parts
    # for simplicity or convert all types to numeric (which might not be ideal
    # for true categoricals)

    # For a comprehensive heatmap, we temporarily convert suitable columns to numeric.
    # We will use only columns that are already numeric or binarized for the
    # plot.
    ...
    fig, axes = plt.subplots(
        len(numerical_features), 1, figsize=(8, 4 * len(numerical_features))
    )
    for i, col in enumerate(numerical_features):
        sns.histplot(
            df, x=col, hue="heart_disease", kde=True, ax=axes[i], palette="viridis"
        )
